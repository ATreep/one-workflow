---
description: Spec-driven delivery flow that reads docs, updates docs first for module changes, then implements inside an Agent Team (primary) or Subagents. This approach may be faster than sdd during development since no tests are required beforehand.
---
...
4. **Implement via `tdd` skill but NOT writting any test — execute through Agent Team (primary) or Subagents**
   ...
   - **Inside each teammate/subagent**: Use the `tdd` skill for development execution.
     - Do NOT follow any test criteria in the `tdd` skill.
   - **Trust but verify**: After teammates/subagents report completion, the main thread inspects the actual diff and do NOT test results before declaring the step done.
...
- **Enforce Agent Team (primary) or Subagents for implementation**. The main thread orchestrates and verifies; all production code are written by teammates/subagents.
...
- Do NOT write/proceed any test beforehand and afterward.
